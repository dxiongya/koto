...
export default function App() {
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      // Cmd+Shift+N — Notebook (agent mode) overlay. Toggle: if already open,
      // closes; if closed, opens the most recent notebook or auto-creates one.
      if (e.metaKey && e.key === 'n' && e.shiftKey) {
        e.preventDefault()
        if (store.notebookOverlayId) {
          store.setNotebookOverlay(null)
          return
        }
        void (async () => {
          const listRes = await window.api.notebook.list()
          const list = listRes.ok ? listRes.data : []
          if (list.length > 0) {
            store.setNotebookOverlay(list[0].id)
          } else {
            const createRes = await window.api.notebook.create('Untitled notebook')
            if (createRes.ok) store.setNotebookOverlay(createRes.data.id)
          }
        })()
        return
      }
