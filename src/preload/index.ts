...
const api = {
  ...
  collector: {
    ...
    dedup: (group?: string) => ipcRenderer.invoke(IpcChannels.COLLECTOR_DEDUP, group),
    sync: {
      getConfig: (groupName: string) => ipcRenderer.invoke(IpcChannels.COLLECTOR_SYNC_GET_CONFIG, groupName),
      setConfig: (groupName: string, config: Record<string, unknown>) => ipcRenderer.invoke(IpcChannels.COLLECTOR_SYNC_SET_CONFIG, groupName, config),
      deleteConfig: (groupName: string) => ipcRenderer.invoke(IpcChannels.COLLECTOR_SYNC_DELETE_CONFIG, groupName),
      listConfigs: () => ipcRenderer.invoke(IpcChannels.COLLECTOR_SYNC_LIST_CONFIGS),
      runNow: (groupName: string) => ipcRenderer.invoke(IpcChannels.COLLECTOR_SYNC_RUN_NOW, groupName),
